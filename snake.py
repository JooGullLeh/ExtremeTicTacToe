...
def keyPressed(event):
    ...
    # Keys only work while game is not over
...
def redrawAll(canvas):
    ...
    if (canvas.data["isGameOver"] == True):
        ...
        canvas.create_text(cx, cy, text="Game Blouses!", font=("Helvetica", 32, "bold"))
...
def printInstructions():
    print """\t\tThe HUMAN CENTIPEDE
            INSTRUCTIONS:
            1. Use the arrow keys to move your body
            2. Connect to the next human (red dot) and grow
            3. Stay on the board!
            4. And don't crash into yourself!
